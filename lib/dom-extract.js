...
// <div> ("...why it wins.<div>. No subscription. $0.20 per response...
// </div>"), purely for styling, not as a separate paragraph. A flat
// "any div/p/... descendant at all" check disqualified the whole
// description as blockish, leaving only that trailing blurb — a strict
// substring — to win the "longest text" heuristic on its own, producing a
// Description starting mid-sentence with a stray leading period instead of
// the real sentence. p/li/ul/ol/table/headings remain hard disqualifiers
// (Google never uses those purely for inline grouping); only bare <div>
// wrappers get this one-level-deeper check.
...
export function extractGoogleAdText(container, baseUrl) {
  ...
  // leaf-text element immediately preceding the URL breadcrumb in document
  // order (e.g. "Semrush" right before "https://www.semrush.com ›
  // on_page › seo_checker"). Worth surfacing as its own item — without it,
  // the website's own downstream AI brand-detection step (which only sees
  // the submitted ad copy text, never the URL/advertiser row) has no
  // genuinely-visible company name or domain to go on and correctly refuses
  // to guess one, leaving its "auditing <brand>'s ad" confirmation blank
  // even though the brand was right there in the source page.
  if (urlLeaf) {
  ...
  // Callouts / sitelinks: confirmed live (2026-07-23) across three real ads
  // that Google renders these in at least three different shapes,
  ...
  //   0 non-empty children: a compact inline text link, no description at
  //     all (e.g. a "SaaS Pricing · Download Your SaaS Playbook · ..."
  //     row) — the anchor's OWN text is the title.
  //   1 non-empty child: a title-only sitelink with a separate, empty-text
  //     icon sibling (e.g. "SEO Checker" / "Try for Free" / "Site Audit"
  //     rows) — that one child's text is the title.
