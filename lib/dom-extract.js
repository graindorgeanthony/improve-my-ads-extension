...
// on a google.com/aclk tracking redirect (confirmed live, 2026-07-23, via
// real markup: href="https://www.google.com/aclk?...&adurl=https://
// productfruits.com/lp/...%3Futm_term%3D..."). One pass of URLSearchParams
// decoding recovers the real, fully UTM-tagged landing page URL exactly as
// Google itself would send the visitor. Some sitelinks instead have an
// already-clean, non-redirect href — those are returned as-is.
export function extractRealDestinationUrl(anchorEl, baseUrl) {
  if (!anchorEl || !anchorEl.href) return "";
  try {
    const url = new URL(anchorEl.href, baseUrl);
